...
async def on_message(message):
    ...
    if message.content.startswith('$stonks'):
        try:
            ...
            await Message.edit(sentMessage.id,'Getting stonks, this may take some time..')
            ...
            await Messageedit(sentMessage.id,'Getting stonks, this may take some time...')
            #formats the message
            toSend = 'Today\'s Data for {stock}\nCurrent Stock Price: {pps:.3f}\nToday\'s High: {high:.3f}\nToday\'s Low: {low:.3f}'
            #sends the message to discord
            await Messageedit(sentMessage.id,toSend.format(stock=ticker,pps=price,high=result['candles'][0]['high'],low=result['candles'][0]['low']))
        except api.APIError:
